...
#' @param resolution takes velue of '\code{high}', '\code{normal}', '\code{low}', '\code{rapid}'
...
#Decompose mean map to radial and residual maps, takes input as "mean" or "decompse"
map_decompose <- function(wafer, resolution="normal", point="off",maps="mean", wlabel ="Wafer Map"){
  ...
  if (maps == "mean") {
   ...
   return(plot.mean)
    ...
    if (point =="off") {
      ...
      #    plot.mean <- ggplot()+
      #     geom_tile(data = wafer.round, aes(x = wafer.round$x,y = wafer.round$y,fill=wafer.round$z))+
      #      xlab(wafer.stat) + ylab('Y(mm)')+ggtitle('Wafer Map')+
      #      theme(axis.line=element_blank(),axis.text.y=element_blank(), axis.text.x=element_blank(),
      #           axis.ticks=element_blank(),
      #          axis.title.y=element_blank(),
      #            axis.title=element_text(size=rel(1)),plot.title = element_text(size=rel(1)),
    ...
    plot.radial <- ggplot(wafer.radial.round, aes(wafer.radial.round$x,wafer.radial.round$y,z=wafer.radial.round$zmean))+
      ...
      xlab(radial.stat) + ylab('Y(mm)')+ggtitle('Radial')+
      theme(axis.line=element_blank(),axis.text.y=element_blank(), axis.text.x=element_blank(),
            axis.ticks=element_blank(),
            axis.title.y=element_blank(),
            axis.title=element_text(size=rel(1)),plot.title = element_text(size = rel(1)),
   ...
   # grid.arrange(bp, arrangeGrob(dp, sc), ncol = 2)
    p2 <- gridExtra::arrangeGrob(plot.radial,plot.residual)
    #plist <- list(map = plot1, radial = plot2, residue = plot3)
    #grid.arrange(bp, dp, vp, sc, ncol=2, nrow =2)
   # plist<-easyGgplot2::ggplot2.multiplot(plot.mean, plot.radial,plot.distribution,plot.residual, ncols=2, nrow=2)
    gridExtra::grid.arrange(plot.mean, p2,ncol=2)
    #return(plist)
  }
